...
public class MarkedAssignment2 {
    public static void main(String[] args) {
         ...
         * 1) production (in kWh) = SOLAR_RADIATION * EFFICIENCY * PANEL_AREA * hours *
         * NUM_OF_PANELS / 1000; //Note the equation in instruction is in Wh, so we
         * divide by 1000 to get kWh as price is per kWh
         * 2) value = production * ELECTRIC_PRICe
         * where,
         * NUM_OF_PANELS = 26;
         * PANEL_HEIGHT = 1; //Units is meters, so no conversion is needed as
         * SOLAR_RADIATION is per m^2
         * SOLAR_RADIATION = 166; //this is in Wh units, hence divide by 1000 in eq 1
         * above to convert to kWh
         * PANEL_WIDTH = 1.7; //Units is meters, so no conversion is needed as
         * SOLAR_RADIATION is per m^2
         * PANEL_AREA = PANEL_WIDTH * PANEL_HEIGHT;
         * EFFICIENCY = 0.2; //(instruction says 20 percent efficiency, so, 20/100 =
         * 0.2)
         * ELECTRIC_PRICE = 0.9; //this is per kWh, so we calculate production in kWh in
         * eq 1
         * DAYS_IN_JUNE_MAX = 30;
         *
         */
